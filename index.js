...
const DB = (() => {
    ...
    return {
        ...
        createHousehold: async (name) => {
            ...
            const newHouseholdData = { name, pin: newPin, categories: ['Esenciales', 'Boludez'] };
            const docRef = await db.collection(HOUSEHOLDS_COLLECTION).add(newHouseholdData);
            return { id: docRef.id, ...newHouseholdData };
        },
        onHouseholdUpdate: (householdId, callback) => {
            return db.collection(HOUSEHOLDS_COLLECTION).doc(householdId).onSnapshot((doc) => {
        ...
        onProductsUpdate: (householdId, callback) => {
            return db.collection(HOUSEHOLDS_COLLECTION).doc(householdId).collection(PRODUCTS_SUBCOLLECTION).onSnapshot((snapshot) => {
        ...
        addProduct: async (householdId, name, category, unit, note, quantity, minimumStock) => {
            const newProductData = { name, category, quantity, unit, note, onShoppingList: false, minimumStock };
...
function renderLoginView() {
    const pinDotsHTML = [...Array(4)].map((_, i) => `
                ...
                <div class="flex flex-col items-center space-y-5" id="numpad">
                    ${[['1','2','3'], ['4','5','6'], ['7','8','9']].map(row => `
                        <div class="flex space-x-5">
                            ${row.map(key => `<button data-key="${key}" class="numpad-btn w-20 h-20 bg-white/50 rounded-full text-3xl font-light text-gray-800 flex items-center justify-center transition-transform active:scale-90">${key}</button>`).join('')}
                        </div>
                    `).join('')}
                    <div class="flex space-x-5">
...
function renderPantryView() {
    ...
    const displayedProducts = (() => {
        ...
        if (state.activeView === View.Shopping) {
            list = list.filter(p => p.onShoppingList);
        }
    ...
    const categories = state.household.categories || [];
...
function renderProductCard(product) {
    ...
    let sliderProps = { min: 0, max: 1, step: 0.01 };
...
function renderSupermarketListView() {
    const initialShoppingListIds = new Set(state.products.filter(p => p.onShoppingList).map(p => p.id));
    const listProducts = state.products.filter(p => initialShoppingListIds.has(p.id));

    const grouped = {};
    const categoryList = [...(state.household.categories || []), 'Sin Categoría'];
...
function showCreateHouseholdModal() {
    ...
    document.getElementById('create-household-form').addEventListener('submit', async (e) => {
        ...
        if (name.trim()) {
            ...
            await handleCreateHousehold(name.trim());
        }
    });
}
...
function showAddProductModal() {
                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <div class="flex rounded-md shadow-sm" id="unit-selector">
                            ${unitOptions.map((u, index) => `<button type="button" data-unit="${u}" class="unit-option px-3 py-2 text-sm font-semibold transition-colors duration-200 ${u === ProductUnit.Units ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'} ${index === 0 ? 'rounded-l-md' : ''} ${index === unitOptions.length - 1 ? 'rounded-r-md' : ''}">${u}</button>`).join('')}
                        </div>
                    </div>
                </div>
                 <div class="mb-4">
                ...
                <div class="mb-6">
                    ...
                    ${categories.length > 0 ? `
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2" id="category-selector">
                            ${categories.map(cat => `<button type="button" data-category="${cat}" class="category-option w-full px-3 py-2 text-sm font-semibold rounded-md transition-colors duration-200 truncate ${cat === categories[0] ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}">${cat}</button>`).join('')}
    ...
    let selectedUnit = ProductUnit.Units;
    ...
    document.getElementById('add-product-form').addEventListener('submit', async (e) => {
        ...
        if(name.trim() && selectedCategory && !isNaN(quantity) && quantity >= 0) {
            await DB.addProduct(state.household.id, name.trim(), selectedCategory, selectedUnit, note.trim(), quantity, minimumStock);
            closeModal();
        }
    });
}
...
function showZeroQuantityModal(product) {
    ...
    `);
    document.getElementById('zero-add-to-list').onclick = () => {
        DB.updateProduct(state.household.id, product.id, { quantity: 0, onShoppingList: true });
        closeModal();
    };
    document.getElementById('zero-delete').onclick = () => {
...
function showMinStockModal(product) {
    ...
    `);
    document.getElementById('min-stock-confirm').onclick = () => {
...
function showHouseholdSettingsModal() {
    let tempCategories = [...(state.household.categories || [])];

    const renderCategories = () => tempCategories.map((cat, index) => `
        <div class="flex items-center space-x-2">
    ...
    document.getElementById('copy-pin').onclick = (e) => {
    ...
    const addNewCategory = () => {
        ...
        if (newCat && !tempCategories.includes(newCat)) {
    ...
    document.getElementById('new-category-input').onkeypress = (e) => e.key === 'Enter' && addNewCategory();

    categoryListDiv.addEventListener('change', e => {
    ...
    document.getElementById('save-settings').onclick = () => {
        const cleaned = tempCategories.map(c => c.trim()).filter(Boolean);
...
function showPurchaseModal(product, checkbox) {
    ...
    const handleConfirm = async () => {
        const numQuantity = parseFloat(qtyInput.value);
        if (isNaN(numQuantity) || numQuantity < 0) return alert('Cantidad inválida.');
        const newTotal = product.quantity + numQuantity;
        await DB.updateProduct(state.household.id, product.id, { quantity: newTotal, onShoppingList: false });
        closeModal();
    };

    confirmBtn.onclick = handleConfirm;
    qtyInput.onkeypress = (e) => e.key === 'Enter' && handleConfirm();
...
async function handleLogin(pin) {
    ...
    if (loggedInHousehold) {
        ...
        state.unsubscribeHousehold = DB.onHouseholdUpdate(state.household.id, household => {
            state.household = household;
            renderApp();
        });
        state.unsubscribeProducts = DB.onProductsUpdate(state.household.id, products => {
    ...
    // No es necesario un renderApp() aquí porque los onSnapshot se encargarán
}
...
async function handleCreateHousehold(name) {
    ...
    // renderApp() no es necesario aquí, los onSnapshot lo manejarán
}
...
function handleQuantityChange(productId, newQuantity) {
    ...
    if (newQuantity === 0) {
        ...
        // La actualización a 0 se hará si el usuario no cancela el modal.
    } else {
        ...
        if (
            ...
            showMinStockModal({...product, quantity: newQuantity});
        }
    }
}
...
function globalEventHandler(e) {
    ...
    if (actionButton) {
        ...
        switch (action) {
            ...
            case 'decrement-quantity': {
                ...
                const change = action === 'increment-quantity' ? 1 : -1;
                let newQuantity;
                ...
                if (product.unit === ProductUnit.Units) {
                    if (change === -1 && product.quantity > 0 && product.quantity <= 1) {
                        ...
                        // Ceil para evitar decimales extraños al sumar
    ...
    if (target.matches('[data-action="buy-item"]') && target.type === 'checkbox') {
        if(target.checked) {
            const productToBuy = state.products.find(p => p.id === target.dataset.productId);
